...
:width(o_width),height(o_height){ // para quando souber apenas o tamanho desejado
	data = new unsigned char[width * height]{0};
}

Channel::Channel(unsigned char *o_data, int o_width, int o_height)
:data(o_data),width(o_width),height(o_height){} // para quando souber o tamanho e a informação
...
void Channel::loadFile(std::string o_path){
	// le o arquivo e define os parâmetros do objeto
	std::ifstream inFile;
	std::string strbuff;
	...
	delete[] data; // caso seja chamado outro loadFile
...
void Channel::loadCSV(std::string o_path){ // le arquivos de 'matriz' para aplicar filtros
	...
	delete[] filter; // caso seja chamado outro loadFilter
...
void Channel::threshold(int threshold){ 
	// transforma em imagem binária com o turnin point de 'threshold'
	for(int k = 0; k < height; ++k){
		for(int l = 0; l < width; ++l){
...
void Channel::thresholdRange(int low, int high){ 
	// zera os valores que estiverem fora do range low - high
	if(low > high){
	...
	for(int i = offset; i < height + offset; ++i){
		for(int j = offset; j < width + offset; ++j){
			...
			for(int k = 0 - offset; k < dimension - offset; ++k){
				for(int l = 0 - offset; l < dimension - offset; ++l){
					sum +=  copy[(i + k)*truewidth+(j + l)] *
					filterX[(k + offset)*dimension + (l + offset)];
				}
			}
			avrg1  = sum/weight;
			avrg1 *= avrg1;
			...
			for(int k = 0 - offset; k < dimension - offset; ++k){
				for(int l = 0 - offset; l < dimension - offset; ++l){
					sum +=  copy[(i + k)*truewidth + (j + l)] * 
					filterY[(k + offset)*dimension + (l + offset)];
				}
			}
			avrg2 = sum / weight;
			avrg2 *= avrg2;
